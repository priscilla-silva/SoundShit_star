...
function analyse() {
  ...
  if (hit) {
    ...
    if (pulsed) {
      ...
      two.bind("update", function(bigCounter) {
          Star.rotation += 0.25 * Math.PI;
          
        })
        .play();

      const currentIntervalMs = intervalMeter.calculate();
      if (currentIntervalMs > 300 && currentIntervalMs < 1000) {
        bigCounter ++
      }
      else if (currentIntervalMs < 300) {
        bigCounter --
      }
      else {
        bigCounter = 0
      }
      document.getElementById('bigCounter').innerHTML = bigCounter;
...
function updateDisplay() {
  ...
  const h = relative * 1200;
  const l = relative * 100;
